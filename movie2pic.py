...
movie_names = [i.strip() for i in config_file.get("DEFAULT", "movie_names").split(',')]
print("movie_names", movie_names)
outdir = config_file.get("DEFAULT", "outdir")
pic_name = config_file.get("DEFAULT", "picname")
interpolation_methods = [i.strip() for i in config_file.get("DEFAULT", "interpolation_methods").split(',')]
numbers_of_rows = [i.strip() for i in config_file.get('DEFAULT', "numbers_of_rows").split(',')]
height_frame = int(config_file.get("DEFAULT", "height_frame"))
x_frames = [i.strip() for i in config_file.get("DEFAULT", "x_frames")]
x_final_pic = config_file.get("DEFAULT", "x_final_pic")
# Flag to remove all black frames at beginning of movie

# for x_frame in ["30"]:
for x_frame in x_frames:
    x_frame = int(x_frame)
                    ...
                    for currentframe in progressbar(range(number_of_frames), "Creating image... ", 40):
                        ...
                        if ret:
                            ...
                            if float(cv2.countNonZero(gray)) < float((frame[:, :, 0]).size / 100) and all_black:
                                ...
                                print("First colored image1 ", str(currentframe))
                                if all_black:
                                    print("First colored image ", str(currentframe))
                                    if x_frame == "":
                                        if x_final_pic == "":
                                            print("x_frame and x_final_pic can't be undefined at"
                                                  " the same time, select at least one")
                                            sys.exit(0)
                                    ...
                                    while left_border < x_orig - 1:
                                        if cv2.countNonZero(cv2.cvtColor(frame[:, left_border:left_border + 1, :],
                                    ...
                                    while right_border > left_border:
                                        ...
                                        if cv2.countNonZero(cv2.cvtColor(frame[:, right_border:right_border + 1, :],
                                                                         cv2.COLOR_BGR2GRAY)) != 0:
                                            break
                                        right_border -= 1
                                    ...
                                    while top_border < height_orig - 1:
                                        ...
                                        if cv2.countNonZero(cv2.cvtColor(frame[top_border:top_border + 1, :, :],
                                                                         cv2.COLOR_BGR2GRAY)) != 0:
                                            break
                                        top_border += 1
                                    ...
                                    while bottom_border > top_border:
                                        ...
                                        if cv2.countNonZero(cv2.cvtColor(frame[bottom_border:bottom_border + 1, :, :],
                                                                         cv2.COLOR_BGR2GRAY)) != 0:
                                            break
                                        bottom_border -= 1
                                        # redefine frame after cropping
                                    print("top_border,bottom_border,left_border,right_border", top_border,
